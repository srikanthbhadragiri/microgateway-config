    ...
    async.parallel([
        function(cb) {
            ...
            if (process.env.EDGEMICRO_LOCAL_PROXY === "1") {
                ...
                loader.getConfigFromRedis(config.edgemicro, globalOptions,config.edge_config.bootstrap, 'config', function(err, body){
        ...
        function(cb) {
             if ( config.edge_config.useRedisCache === true ) { //retrieve info from redis db
                loader.getConfigFromRedis(config.edgemicro, globalOptions,config.edge_config.products, 'products', function(err, body){
                    const response = { statusCode: 200, statusMessage: 'Downloaded from redis' };
        ...
        function(cb) {
            if ( config.edge_config.useRedisCache === true ) { //retrieve info from redis db
                loader.getConfigFromRedis(config.edgemicro, globalOptions, config.edge_config.jwt_public_key, 'jwt_public_key', function(err, body){
                    const response = { statusCode: 200, statusMessage: 'Downloaded from redis' };
...
Loader.prototype.connectRedisDb = function (config, cb) {
    ...
    if ( !this.redisClient ) {
        ...
        debug('creating redisClient with port: %d, host: %s options: %j ', port, host, options);
    ...
    this.redisClient.on('connect',  () => {
        debug('redisClient is connected');
 ...
 */
Loader.prototype.getConfigFromRedis = function (config, options, proxy, suffixKey, cb) {
    const proxyName = url.parse(proxy).pathname.split('/')[1];  // get the proxy name from the url
    try {
        this.connectRedisDb(config, (err) => {
            if (err) {
                cb(err);
            } else {
                let key = options.org + REDIS_KEY_ORG_ENV_DELIMETER + options.env + REDIS_KEY_ORG_ENV_DELIMETER + proxyName;
                if ( suffixKey && typeof suffixKey === 'string' ){
                    key += REDIS_KEY_SUFFIXKEY_DELIMETER+suffixKey;
                }
                debug("Reading redis data from key: %s", key);
                writeConsoleLog('info',{component: CONSOLE_LOG_TAG_COMP},'Reading redis data from key: '+key);
