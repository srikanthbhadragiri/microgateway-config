  ...
  if (config.quotas) {
    ...
    Object.keys(config.quotas).forEach(key => {
      if (  key === 'failOpen') {
        ...
        assert(config.quotas[key] === 'default' ||
        config.quotas[key] === 'hour' ||
        config.quotas[key] === 'minute' ||
        config.quotas[key] === 'day' ||
        config.quotas[key] === 'week', 'invalid value in config.quotas: ' + key +
                ', valid values are hour, minute, day, week, & default');
        assert(typeof config.quotas[key] === 'string', 'config.quotas.' + key + ' is not an object');
      } else if (key === 'bufferSize'){
        assert(typeof config.quotas[key] === 'number', 'config.quotas.bufferSize ' + config.quotas.bufferSize + ' is not a number');
        assert(+config.quotas[key] >= 0, 'config.quotas.' + config.quotas['timeUnit'] + '.bufferSize must be greater than or equal to zero');
      }
    })  
  }
  return config;
