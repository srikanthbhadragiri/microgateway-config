...
module.exports.validate = function validate(config) {
  ...
  if (!process.env.EDGEMICRO_LOCAL && !process.env.EDGEMICRO_LOCAL_PROXY) {
    ...
    if (config.edge_config.hasOwnProperty('useRedisCache')) {
      assert(typeof config.edge_config.useRedisCache === 'boolean', 'config.edge_config.useRedisCache is not a boolean');
    }  
  }
